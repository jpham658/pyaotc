...
void test_create_range()
{
    ...
    word val;
    assert(range_index(r, 0, &val) && val == 2);
    assert(range_index(r, 3, &val) && val == 8);
...
void test_create_range_with_negative_step()
{
    ...
    assert(range_len(r) == 4);
    word val;
    assert(range_index(r, 0, &val) && val == 10);
    assert(range_index(r, 3, &val) && val == 4);
    Iterator *it = range_iter(r);
...
void test_range_index()
{
    ...
    word val;
    assert(range_index(r, 0, &val) && val == 0);
    assert(range_index(r, 4, &val) && val == 4);
    assert(!range_index(r, 5, &val));
}
...
void test_range_index_with_negative_index()
{
    Range *r = create_range(2, 10, 2);
    word val;
    assert(range_index(r, -1, &val) && val == 8);
    assert(range_index(r, -2, &val) && val == 6);
    assert(!range_index(r, -5, &val));
}
